...
import { deleteAdmission } from '../../../../actions/admission';
...
export default function AdmissionDelete(props) {
    const classes = useStyles();
    const {modalPopup,auth} = useSelector(state => state);
    const dispatch = useDispatch();
    const onSubmit = (e) =>{
      dispatch(deleteAdmission(modalPopup.data.Id,auth.tokenId));
    }
    const onCancel = (e) =>{
  ...
  return (
    <form className={classes.root} noValidate autoComplete="off" >
        <h3>You want to delete admission record. Are you sure?</h3>
